...
pImageBuffer=LoadJPG(fichero,w,h,BytesPerPixel)

Dim micolor As UInteger
Dim reg As Integer
Dim reg2 As Integer=0
Dim As Integer xx,yy,zz
xx=0:yy=0:zz=0
Open "texturas.bin" For binary As 2
reg=Lof(2)+1 ' sigo sumando tras el final
...
if pImageBuffer then
  ...
  for y as integer=0 to h-1
    for x as integer=0 to w-1
    	micolor=rgb(pRGB[0],pRGB[1],pRGB[2])
      pset(x,y),rgb(pRGB[0],pRGB[1],pRGB[2])
      	Put #2,reg+xx+yy,micolor
      	xx+=4
      	If xx>255 Then
      		xx=0
      		yy+=16384
    ...
    yy=0
    zz+=1
    If zz=64 Then zz=0:reg2+=16384*(w/64):reg=reg2+1 Else reg+=256
  Next

  'Dim as ubyte ptr pRGB=pImageBuffer
  'For y as integer=0 to h-1
  '  for x as integer=0 to w-1
  '  	micolor=Point(pRGB[0],pRGB[1],pRGB[2])
  '    	Put #2,reg,micolor
  '    	reg+=4
  '    	
  '      pRGB+=BytesPerPixel+xx+(yy*w):If xx=64 Then xx=0:yy+=1:If yy=64 Then 
  '  next
  'Next
  screenunlock
